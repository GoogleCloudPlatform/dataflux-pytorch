    ...
    def forward(
        ...
        if input_pos is None:  # proxy for use_cache=False
            ...
            if not self.kv_caches:
                ...
                cache_shape = (B, self.config.n_head, max_seq_length,
                               head_size)
                self.kv_caches = [(torch.zeros(cache_shape,
                                               device=x.device,
                                               dtype=x.dtype),
            ...
            for i, block in enumerate(self.transformer.h):
                x, self.kv_caches[i] = block(x, rope, mask, max_seq_length,
...
class Block(nn.Module):
    ...
    def forward(
        ...
        h, new_kv_cache = self.attn(self.rms_1(x), rope, mask, max_seq_length,
                                    input_pos, kv_cache)
        x = x + h
