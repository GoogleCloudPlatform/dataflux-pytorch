...
def quantization(mode: str = None):
    ...
    if mode == 'llm.int8':
        ...
        from .quantization import ColBlockQuantizedLinear
        quantized_linear_cls = functools.partial(ColBlockQuantizedLinear,
                                                 bits=4,
                                                 tile_cols=-1)
    elif mode == 'gptq.int8':
        from .quantization import ColBlockQuantizedLinear
...
class NotYetLoadedTensor:
    ...
    def rebuild_parameter(cls,
        ...
        if isinstance(data, NotYetLoadedTensor):
            ...
            def _load_tensor():
                t = old_lt()
                return torch._utils._rebuild_parameter(t, requires_grad,
                                                       backward_hooks)

            data._load_tensor = _load_tensor
            return data
        return torch._utils._rebuild_parameter(data, requires_grad,
                                               backward_hooks)
    ...
    def rebuild_tensor_v2(
        ...
        *,
        ...
        return NotYetLoadedTensor(metatensor, archiveinfo, storageinfo,
                                  rebuild_args)
    ...
    def _load_tensor(self):
        ...
        uts = (self.archiveinfo.zipfile_context.zf.get_storage_from_record(
            f"data/{fn}",
        ...
        with warnings.catch_warnings():
            ...
            storage = torch.storage.TypedStorage(wrap_storage=uts,
                                                 dtype=self.metatensor.dtype,
                                                 _internal=True)
    ...
    def __torch_function__(cls, func, types, args=(), kwargs=None):
        if kwargs is None:
            kwargs = {}
        loaded_args = [
            (a._load_tensor() if isinstance(a, NotYetLoadedTensor) else a)
            for a in args
        ]
...
class LazyLoadingUnpickler(pickle.Unpickler):
    ...
    def find_class(self, module, name):
        ...
        if module == "torch._utils" and name == "_rebuild_tensor_v2":
            return functools.partial(NotYetLoadedTensor.rebuild_tensor_v2,
                                     archiveinfo=self)
        elif module == "torch._tensor" and name == "_rebuild_from_type_v2":
            return functools.partial(NotYetLoadedTensor.rebuild_from_type_v2,
                                     archiveinfo=self)
        elif module == "torch._utils" and name == "_rebuild_parameter":
            return functools.partial(NotYetLoadedTensor.rebuild_parameter,
                                     archiveinfo=self)
        return res
...
class SavingProxyForStorage:
    ...
    def __init__(self, obj, saver, protocol_version=5):
        ...
        self.saver = saver
        if not (isinstance(obj, torch.storage.TypedStorage)
...
class SavingProxyForTensor:
    ...
    def __init__(self, tensor, saver, protocol_version=5):
        ...
        self.reduce_ret_fn, (
            storage,
            *other_reduce_args) = tensor.__reduce_ex__(protocol_version)
        assert isinstance(
            storage, torch.storage.TypedStorage), "Please check for updates"
        ...
        if isinstance(obj,
            ...
            if storage.data_ptr() != 0:
                if storage.data_ptr() in self.storage_dtypes:
                    if storage_dtype != self.storage_dtypes[
                            storage.data_ptr()]:
                        raise RuntimeError(
